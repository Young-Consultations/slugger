...
def _make_contract() -> AgentContract:
    return AgentContract(
        ...
        inputs=[
            FieldSchema(name='requirements', description='Requirements document', required=True),
            FieldSchema(name='architecture', required=False),
        ],
        outputs=[
            FieldSchema(name='source_code', description='Generated source', required=True),
        ],
    )
...
def test_contract_validate_inputs_valid() -> None:
    ...
    errors = contract.validate_inputs({'requirements': '# Requirements', 'architecture': 'none'})
    assert errors == []
