...
class FakeVisionAgent(BaseAgent):
    def __init__(self) -> None:
        super().__init__(metadata=AgentMetadata(name='product_vision_agent', version='1.0.0', description='fake', category='planning', outputs=['product_vision']), capabilities=[AgentCapability(name='product_vision', description='fake')])
...
class FakeRequirementsAgent(BaseAgent):
    def __init__(self) -> None:
        super().__init__(metadata=AgentMetadata(name='requirements_agent', version='1.0.0', description='fake', category='planning', outputs=['requirements']), capabilities=[AgentCapability(name='requirements', description='fake')])
...
def test_workflow_engine_runs_recipe() -> None:
    ...
    assert [artifact.name for artifact in result.artifacts] == ['product_vision', 'requirements']
