...
class TestWorkflowEngineApprovalGate:
    ...
    def test_auto_approve_does_not_block_execution(self, tmp_path) -> None:
        ...
        recipe_dir.mkdir()
        (recipe_dir / "approval-test.yaml").write_text(
            yaml.dump(recipe), encoding="utf-8"
        )
        ...
        engine = WorkflowEngine(
            recipe_dir, WorkflowParser(WorkflowValidator()), executor, store
        )
    ...
    def test_non_auto_approve_pauses_execution(self, tmp_path) -> None:
        ...
        recipe_dir = tmp_path / "recipes"
        recipe_dir.mkdir()
        (recipe_dir / "approval-pause-test.yaml").write_text(
            yaml.dump(recipe), encoding="utf-8"
        )
        ...
        engine = WorkflowEngine(
            recipe_dir, WorkflowParser(WorkflowValidator()), executor
        )
