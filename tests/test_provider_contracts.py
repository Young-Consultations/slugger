...
def mock_config() -> ProviderConfig:
    return ProviderConfig(
        name="test-mock", provider_type=ProviderType.MOCK, model="mock-v1"
    )
...
class TestMockProviderTypedContracts:
    def test_generate_returns_generation_result(
        ...
        request = GenerationRequest(
            prompt="Write a hello world function", language="Python"
        )
    ...
    def test_refactor_returns_refactor_result(
        self, mock_provider: MockProvider
    ) -> None:
        request = RefactorRequest(
            code="x = 1", instruction="rename to counter", language="Python"
        )
        result = mock_provider.refactor(request)
        assert isinstance(result, RefactorResult)
        assert result.refactored_code == "x = 1"

    def test_embed_typed_returns_embedding_result(
        self, mock_provider: MockProvider
    ) -> None:
    ...
    def test_health_check_returns_health_result(
        self, mock_provider: MockProvider
    ) -> None:
