...
class TestProjectInput:
    ...
    def test_as_metadata_returns_plain_strings(self) -> None:
        pi = ProjectInput(
            ...
            coding_agent=CodingAgent.ANTHROPIC,
        )
        meta = pi.as_metadata()
        assert meta == {
            "idea": "Todo list app",
    ...
    def test_build_rejects_invalid_coding_agent(self) -> None:
        with pytest.raises(SystemExit):
            self.parser.parse_args(
                ["build", "An idea", "--platform", "web", "--coding-agent", "gemini"]
            )
...
class TestMainBuildCommand:
    def _make_fake_result(
        self, workflow_name: str = "full-sdlc", status: str = "succeeded"
    ) -> MagicMock:
        result = MagicMock()
    ...
    @patch("cli.main.Bootstrap")
    def test_build_command_output_includes_idea_and_platform(
        self, mock_bootstrap: MagicMock, capsys
    ) -> None:
        fake_slugger = MagicMock()
        fake_slugger.build.return_value = self._make_fake_result()
    ...
    @patch("cli.main.Bootstrap")
    def test_build_command_forwards_project_input_to_slugger(
    ...
    @patch("cli.main.Bootstrap")
    def test_build_command_forwards_workflow_override(
        self, mock_bootstrap: MagicMock
    ) -> None:
        fake_slugger = MagicMock()
        fake_slugger.build.return_value = self._make_fake_result(
...
class TestApprovalCLI:
    def test_cli_approvals_list(self, capsys) -> None:
        ...
        fake_store.list_requests.return_value = [
