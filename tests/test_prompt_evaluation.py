...
from validators import PromptEvaluator, PromptTemplateValidator
...
class TestPromptTemplateDiscovery:
    ...
    def test_load_template_returns_non_empty_string(
...
class TestPlaceholderExtraction:
    ...
    def test_extract_placeholders_deduplicates(
        self, evaluator: PromptEvaluator, tmp_path: Path
    ) -> None:
        dup_template = tmp_path / "dup.md"
        dup_template.write_text(
            "{{ foo }} and {{ foo }} again, {{ bar }}", encoding="utf-8"
        )
        ...
        for name, variables in _VARIABLES.items():
            ...
            for value in variables.values():
                assert value in rendered, (
                    f"Variable value {value!r} not found in rendered {name!r}"
                )

    def test_unresolved_placeholders_remain_when_variable_missing(
        self, evaluator: PromptEvaluator
    ) -> None:
...
class TestPromptQualityGates:
    ...
    def test_rendered_prompt_contains_required_sections(
        ...
        result = evaluator.evaluate(
            template_name, _VARIABLES[template_name], extra_sections=sections
        )
        assert result.validation.valid, (
    ...
    def test_missing_required_section_fails_validation(self, tmp_path: Path) -> None:
        ...
        ev = PromptEvaluator(
            tmp_path, PromptTemplateValidator(required_sections=["MISSING"])
        )
...
class TestPromptEvaluationResult:
    ...
    def test_result_reports_no_missing_variables_when_all_supplied(
        self, evaluator: PromptEvaluator
    ) -> None:
        result = evaluator.evaluate(
            "requirements-prompt", _VARIABLES["requirements-prompt"]
        )
    ...
    def test_result_template_name_matches_input(
        self, evaluator: PromptEvaluator
    ) -> None:
        result = evaluator.evaluate("requirements-prompt", {})
...
class TestEvaluateAllRegressionSweep:
    ...
    def test_all_templates_pass_with_canonical_variables(
        self, evaluator: PromptEvaluator
    ) -> None:
    ...
    def test_all_templates_produce_non_empty_renders(
        self, evaluator: PromptEvaluator
    ) -> None:
        results = evaluator.evaluate_all(_VARIABLES)
        for name, result in results.items():
            assert result.rendered.strip(), (
                f"Template {name!r} rendered to empty string"
            )
