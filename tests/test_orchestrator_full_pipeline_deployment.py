"""Test the orchestrator running the full pipeline with all eight agents.

This integration test demonstrates the orchestrator coordinating Requirements,
Business Analyst, Architecture, Planning, Coding, Testing, Documentation,
and Deployment agents in sequence.
"""
...
def test_orchestrator_full_pipeline_to_deployment():
    """Run full pipeline: request -> ... -> documentation -> deployment."""
    ...
    orch.register_agent(DeploymentAgent())

    # Define the full pipeline
    pipeline = ["requirements", "business_analyst", "architecture", "planning", "coding", "testing", "documentation", "deployment"]
    ...
    # Final artifact should be the deployment config
    assert "artifact" in result, "Final artifact should be present"
    assert result["artifact"]["type"] == "deployment_config", "Final artifact should be deployment config"

    # Verify memory registered all artifacts
    assert len(orch.memory.artifact_registry) == 8, "Should have registered 8 artifacts"
    ...
    assert "deployment_config_v1" in orch.memory.artifact_registry

    # Verify workflow progress tracking
    assert len(orch.workflow.progress) == 8, "Should have 8 execution progress records"
