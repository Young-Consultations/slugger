...
class TestSQLiteArtifactStore:
    ...
    def test_find_by_name(self, tmp_path: Path) -> None:
        ...
        store.create(DocumentArtifact(artifact_id='a2', name='requirements', content='R1'))
...
class TestDurableApprovalStore:
    def test_save_and_retrieve(self, tmp_path: Path) -> None:
        ...
        record = ApprovalRecord(
            record_id='r1', checkpoint_name='pre-deploy', run_id='run-1',
    ...
    def test_survives_restart(self, tmp_path: Path) -> None:
        ...
        store.save(ApprovalRecord(record_id='r1', checkpoint_name='cp', run_id='run-1', decision=ApprovalDecision.APPROVED, approver='bob'))
    ...
    def test_has_approval_false_when_pending(self, tmp_path: Path) -> None:
        ...
        from workflow.approvals import ApprovalRecord, ApprovalDecision
        store = DurableApprovalStore(tmp_path / 'approvals.db')
        store.save(ApprovalRecord(record_id='r1', checkpoint_name='cp', run_id='run-1', decision=ApprovalDecision.PENDING))
    ...
    def test_pending_approvals_returned_on_resume(self, tmp_path: Path) -> None:
        ...
        from workflow.approvals import ApprovalRecord, ApprovalDecision
        store = DurableApprovalStore(tmp_path / 'approvals.db')
        store.save(ApprovalRecord(record_id='r1', checkpoint_name='deploy', run_id='run-1', decision=ApprovalDecision.PENDING))
...
class TestKnowledgeContextInjection:
    ...
    def test_knowledge_context_is_injectable(self) -> None:
        ...
        ctx = ExecutionContext(
            project_id='p1', workflow_name='wf', step_name='s',
...
class TestFullSDLCRecipe:
    def test_v2_recipe_exists(self) -> None:
        from pathlib import Path
        recipe_path = Path(__file__).parent.parent / 'workflow' / 'recipes' / 'full-sdlc-v2.yaml'
...
class TestGitHubAutomation:
    ...
    def test_mock_github_service_creates_pr(self) -> None:
        from services.github import MockGitHubService
        svc = MockGitHubService()
        pr = svc.create_pull_request('Feature branch', 'main', 'feature/test', 'Test PR')
...
class TestAcceptanceScenarios:
    def test_idea_to_artifacts_pipeline(self) -> None:
        ...
        ctx = ExecutionContext(project_id='acc-1', workflow_name='sdlc', step_name='vision', metadata={'idea': idea})
        ...
        ctx2 = ExecutionContext(project_id='acc-1', workflow_name='sdlc', step_name='req',
                                metadata={'idea': idea}, inputs={'product_vision': vision_artifacts[0]})
        req_artifacts = RequirementsAgent().execute(ctx2)
        assert len(req_artifacts) == 1
        assert 'Requirements' in req_artifacts[0].content

        ctx3 = ExecutionContext(project_id='acc-1', workflow_name='sdlc', step_name='stories',
                                metadata={'idea': idea}, inputs={'requirements': req_artifacts[0]})
    ...
    def test_manifest_to_workspace_pipeline(self, tmp_path: Path) -> None:
        ...
        manifest = make_cli_manifest('acc-cli-1', 'TaskTracker', 'CLI task tracking app')
    ...
    def test_remediation_loop_with_seeded_defect(self) -> None:
        ...
        findings = [
            Finding('f1', FindingSeverity.HIGH, FindingCategory.CODE_REVIEW, 'Missing null check'),
        ]
