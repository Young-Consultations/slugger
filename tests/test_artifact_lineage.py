...
def _build_sdlc_graph() -> LineageGraph:
    ...
    nodes = [
        ...
        ArtifactLineageNode('req-1', 'requirements', SdlcStage.REQUIREMENTS, parent_ids=['idea-1']),
        ArtifactLineageNode('story-1', 'user_stories', SdlcStage.STORIES, parent_ids=['req-1']),
        ArtifactLineageNode('arch-1', 'architecture', SdlcStage.ARCHITECTURE, parent_ids=['req-1']),
        ArtifactLineageNode('adr-1', 'adr', SdlcStage.ADR, parent_ids=['arch-1']),
        ArtifactLineageNode('task-1', 'tasks', SdlcStage.TASKS, parent_ids=['story-1', 'arch-1']),
        ArtifactLineageNode('code-1', 'source_code', SdlcStage.CODE, parent_ids=['task-1']),
        ArtifactLineageNode('test-1', 'tests', SdlcStage.TESTS, parent_ids=['code-1']),
        ArtifactLineageNode('release-1', 'release', SdlcStage.RELEASE, parent_ids=['test-1']),
    ]
    for node in nodes:
        graph.add(node)
    return graph
