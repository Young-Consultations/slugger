...
def test_standards_by_category(repo: StandardsRepository) -> None:
    repo.add_standard(
        EngineeringStandard("SEC-001", "Validation", "desc", StandardCategory.SECURITY)
    )
    repo.add_standard(
        EngineeringStandard("COD-001", "Type hints", "desc", StandardCategory.CODING)
    )
...
def test_patterns_by_category(repo: StandardsRepository) -> None:
    ...
    repo.add_pattern(
        Pattern("P2", "Prompt Chain", "desc", StandardCategory.PROMPT_ENGINEERING)
    )
...
def test_summary(repo: StandardsRepository) -> None:
    repo.add_standard(
        EngineeringStandard("S1", "T1", "D1", StandardCategory.SECURITY, mandatory=True)
    )
