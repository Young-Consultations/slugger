...
def test_create_issue(svc: MockGitHubService) -> None:
    issue = svc.create_issue('Bug: null pointer', 'Steps to reproduce...', labels=['bug'])
...
def test_create_milestone(svc: MockGitHubService) -> None:
    ms = svc.create_milestone('v1.0', description='First release', due_on='2026-12-31T00:00:00Z')
...
def test_list_workflow_runs_with_runs(svc: MockGitHubService) -> None:
    svc.workflow_runs = [
        GitHubWorkflowRun(id=1, name='CI', status='completed', conclusion='success', workflow_id=10),
