...
class TestCommandEvidence:
    def test_passed_when_exit_code_zero(self) -> None:
        e = CommandEvidence(
            command="test", exit_code=0, stdout="ok", stderr="", duration_ms=1.0
        )
        assert e.passed is True

    def test_failed_when_exit_code_nonzero(self) -> None:
        e = CommandEvidence(
            command="test", exit_code=1, stdout="", stderr="fail", duration_ms=1.0
        )
...
class TestBuildRunnerIntegration:
    ...
    def test_build_runner_uses_allowed_commands(
        ...
        result = runner.run(
            tmp_path, ["pytest tests/ -q", "pip install ."], app_id="good-app"
        )
...
def _finding(
    fid: str, severity=FindingSeverity.MEDIUM, category=FindingCategory.CODE_REVIEW
) -> Finding:
    return Finding(
        finding_id=fid,
        severity=severity,
...
class TestBoundedRemediationLoop:
    ...
    def test_escalated_when_attempt_fn_always_fails(self) -> None:
        ...
        with pytest.raises(RemediationExhaustedError) as exc_info:
            loop.process(findings, attempt_fn=lambda f: False)
    ...
    def test_all_findings_have_lifecycle_status(self) -> None:
        ...
        with pytest.raises(RemediationExhaustedError):
            loop.process(
                findings, attempt_fn=lambda f: f.severity == FindingSeverity.LOW
            )
        for f in findings:
    ...
    def test_bounded_remediation_exhausted(self) -> None:
        ...
        with pytest.raises(RemediationExhaustedError) as exc_info:
            loop.process([finding], attempt_fn=lambda f: False)
