...
def test_profile_inheritance(tmp_path) -> None:
    ...
    manager.register(
        Profile("ci", extends="base", overrides={"observability.log_level": "WARNING"})
    )
    settings = manager.activate("ci")
...
def test_child_overrides_parent(tmp_path) -> None:
    ...
    manager.register(
        Profile("child", extends="base", overrides={"environment": "child"})
    )
...
def test_profiles_yaml_round_trip(tmp_path) -> None:
    ...
    manager.register(
        Profile(
            "staging", overrides={"environment": "staging"}, description="Staging env"
        )
    )
