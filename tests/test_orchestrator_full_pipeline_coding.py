"""Test the orchestrator running the full pipeline with all five agents.

This integration test demonstrates the orchestrator coordinating Requirements,
Business Analyst, Architecture, Planning, and Coding agents in sequence.
"""
...
def test_orchestrator_full_pipeline_to_coding():
    """Run full pipeline: request -> ... -> planning -> coding."""
    ...
    pipeline = ["requirements", "business_analyst", "architecture", "planning", "coding"]
    ...
    # Final artifact should be the code scaffold
    assert "artifact" in result, "Final artifact should be present"
    assert result["artifact"]["type"] == "code_scaffold", "Final artifact should be code scaffold"

    # Verify memory registered all artifacts
    assert len(orch.memory.artifact_registry) == 5, "Should have registered 5 artifacts"
    ...
    assert len(orch.workflow.progress) == 5, "Should have 5 execution progress records"
