...
def _context(idea: str = 'task manager', design_id: str | None = None) -> ExecutionContext:
    meta = {'idea': idea}
    if design_id:
...
class TestDesignBrief:
    ...
    def test_brief_includes_requirements_from_context(self) -> None:
        ...
        req_artifact = DocumentArtifact(artifact_id='r1', name='requirements', content='REQ-001: Login')
...
class TestDesignManifest:
    def test_manifest_has_screens(self) -> None:
        manifest = _build_design_manifest('App', 'd1', 'https://export.canva.com/d1.pdf', 'abc123')
    ...
    def test_manifest_links_brief_hash(self) -> None:
        manifest = _build_design_manifest('App', 'd1', 'https://export.canva.com/d1.pdf', 'deadbeef')
...
class TestCanvaDesignAgentCC006:
    ...
    def test_brief_hash_consistent_across_artifacts(self) -> None:
        ...
        agent = CanvaDesignAgent(svc)
        artifacts = agent.execute(_context())
        hashes = {a.extra.get('brief_hash') for a in artifacts if 'brief_hash' in a.extra}
