...
def _make_context(
    idea: str = "test idea", chatgpt_service=None, **extra_inputs
) -> ExecutionContext:
...
class TestRequirementsWithChatGPT:
    ...
    def test_includes_vision_in_prompt(self):
        ...
        svc = MockChatGPTService()
        vision = DocumentArtifact(
            artifact_id="a1", name="product_vision", content="# Vision\nGreat app"
        )
        ctx = _make_context(idea="task manager", chatgpt_service=svc)
...
class TestProjectPlanWithChatGPT:
    ...
    def test_plan_preserves_upstream_traceability_ids(self):
        ...
        req = DocumentArtifact(
            artifact_id="req-1", name="requirements", content="REQ-123: Login"
        )
        stories = DocumentArtifact(
            artifact_id="story-1",
            name="user_stories",
            content="STORY-456: As a user...",
        )
...
class TestStepExecutorPassesChatGPT:
    def test_executor_injects_chatgpt_into_context(self):
        ...
        step_def = WorkflowStepDefinition(
            name="step", agent="test_agent", inputs=[], outputs=[], quality_gates=[]
        )
