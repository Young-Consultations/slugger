...
class ProjectMaterializer:
    ...
    def materialize(self, manifest: AppManifest, workspace_id: str | None = None) -> MaterializationResult:
        ...
        if active_dir.exists():
            if not (active_dir / '.slugger').exists():
                return MaterializationResult(
                    success=False,
                    errors=[f'Directory {active_dir} already exists and is not a Slugger workspace'],
                )
        ...
        try:
            ...
            if staging_dir.exists():
                ...
                shutil.copytree(staging_dir, failed_dir / 'failed_staging', dirs_exist_ok=True)
                shutil.rmtree(staging_dir, ignore_errors=True)
            record.state = WorkspaceState.FAILED
            record.errors.append(str(exc))
            return MaterializationResult(success=False, workspace=record, errors=[str(exc)])

        # Write Slugger marker file
        (staging_dir / '.slugger').write_text(f'app_id={manifest.app_id}\n', encoding='utf-8')
    ...
    def resume(self, manifest: AppManifest, workspace_id: str | None = None) -> MaterializationResult:
        ...
        return MaterializationResult(success=not errors, workspace=record, errors=errors)
    ...
    def _write_files(self, files: list[FileEntry], staging_dir: Path) -> list[FileInventoryEntry]:
