...
class ProjectPlanAgent(BaseAgent):
    ...
    def _execute(self, context: ExecutionContext):
        ...
        artifact = self.create_artifact(
            context, "project_plan", content, DocumentArtifact
        )
    ...
    def _generate_plan(
        ...
        if svc is not None:
            prompt = _PROJECT_PLAN_PROMPT.format(
                idea=idea, requirements=requirements or "(not available)"
            )
            try:
                result = svc.execute(
                    prompt,
                    system="You are a project manager writing structured project plans.",
                )
                return f"# Project Plan\n\n{result.response}"
            except Exception as exc:  # noqa: BLE001
                _LOG.warning(
                    "ChatGPT project plan generation failed, using template: %s", exc
                )
