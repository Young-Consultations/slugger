...
class RequirementsAgent(BaseAgent):
    ...
    def _execute(self, context: ExecutionContext):
        ...
        artifact = self.create_artifact(
            context, "requirements", content, DocumentArtifact
        )
    ...
    def _generate_requirements(
        self, idea: str, vision: str, context: ExecutionContext
    ) -> str:
        svc = context.chatgpt_service
        if svc is not None:
            prompt = _REQUIREMENTS_PROMPT.format(
                idea=idea, vision=vision or "(not available)"
            )
            try:
                result = svc.execute(
                    prompt,
                    system="You are a business analyst writing precise software requirements.",
                )
                return f"# Requirements\n\n{result.response}"
            except Exception as exc:  # noqa: BLE001
                _LOG.warning(
                    "ChatGPT requirements generation failed, using template: %s", exc
                )
