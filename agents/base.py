...
class BaseAgent(IAgent, ABC):
    ...
    def validate_inputs(self, context: ExecutionContext) -> None:
        ...
        if missing:
            raise AgentError(
                f"Missing required inputs for {self.metadata.name}: {', '.join(missing)}"
            )
    ...
    def on_after_execute(
        self, context: ExecutionContext, artifacts: list[Artifact]
    ) -> None:
    ...
    def create_artifact(
        ...
        name: str,
        content: str,
        artifact_class: Type[Artifact] = DocumentArtifact,
        *,
    ...
    def _emit(self, name: str, context: ExecutionContext, **payload: object) -> None:
        context.add_event(
            ExecutionEvent(name=name, payload={"agent": self.metadata.name, **payload})
        )
        if self._observer is not None:
            self._observer.emit(
                Event(name=name, payload={"agent": self.metadata.name, **payload})
            )
