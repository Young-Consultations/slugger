...
class ManifestValidationResult:
    ...
    def add_warning(self, field: str, message: str) -> None:
        self.warnings.append(
            ManifestValidationError(field=field, message=message, severity="warning")
        )
...
def validate_app_manifest(manifest: AppManifest) -> ManifestValidationResult:
    ...
    if total_size > MAX_TOTAL_SIZE_BYTES:
        result.add_error(
            "files",
            f"Total file size {total_size} exceeds {MAX_TOTAL_SIZE_BYTES} bytes",
        )
...
def _validate_required_files(
    manifest: AppManifest, result: ManifestValidationResult
) -> None:
    ...
    if manifest.template == AppTemplate.CLI:
        ...
        if not has_src:
            result.add_error(
                "files", "CLI template requires at least one source .py file"
            )
        if not has_test:
            result.add_error(
                "files", "CLI template requires at least one test file under tests/"
            )
        ...
        if not has_app:
            result.add_error(
                "files", "FastAPI template requires a main.py or app.py file"
            )
        if not has_test:
            result.add_error(
                "files", "FastAPI template requires at least one test file under tests/"
            )
    else:
        required = set()
...
def _validate_file_entry(
    entry: FileEntry, paths_seen: set[str], result: ManifestValidationResult
) -> None:
    ...
    if _CREDENTIAL_NAMES.search(path):
        result.add_error(
            f"files[{path}]", f"Credential/secret filename not allowed: {path!r}"
        )
    ...
    if entry.size_bytes > MAX_FILE_SIZE_BYTES:
        result.add_error(
            f"files[{path}]",
            f"File size {entry.size_bytes} exceeds {MAX_FILE_SIZE_BYTES} bytes",
        )
