...
class ArtifactValidator(BaseValidator):
    def validate(self, target: Artifact, **kwargs: object) -> ValidationResult:
        ...
        if not target.artifact_id:
            errors.append(
                ValidationError(field="artifact_id", message="Artifact id is required.")
            )
        if not target.name:
            errors.append(
                ValidationError(field="name", message="Artifact name is required.")
            )
        if require_content and not target.content.strip():
            errors.append(
                ValidationError(
                    field="content", message="Artifact content is required."
                )
            )
        if (
            target.name == "generated_code"
            and target.format == "json"
            and target.content.strip()
        ):
            try:
                ...
                errors.append(
                    ValidationError(
                        field="content",
                        message=f"Generated code must be a valid AppManifest: {exc}",
                    )
                )
