...
class OpenAIProvider(BaseProvider):
    ...
    def _auth_headers(self) -> dict[str, str]:
        if not self._api_key:
            raise ProviderError(
                "OpenAI API key is not configured. Set OPENAI_API_KEY or supply api_key in secrets."
            )
        return {
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            raise ProviderError(
                f"OpenAI embeddings API error {exc.response.status_code}: {exc.response.text}"
            ) from exc
        except requests.RequestException as exc:
