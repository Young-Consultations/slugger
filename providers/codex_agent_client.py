...
class CodexCliAdapter(ICodexAgentClient):
    ...
    def __init__(
        ...
        *,
        ...
        self.model = model
        self.api_key = api_key or os.getenv('CODEX_API_KEY') or os.getenv('OPENAI_API_KEY')
        if not self.api_key:
    ...
    def _run_exec(self, prompt: str, workspace: CodexWorkspace, *, session_id: str) -> CodexTaskResult:
        ...
        self._sessions[session_id].append(
            CodexEvent(
                ...
                payload={'command': ' '.join(command), 'returncode': completed.returncode},
                session_id=session_id,
            )
        )
        summary = (completed.stdout or completed.stderr or 'Codex CLI completed').strip()
        if completed.returncode != 0:
            self._sessions[session_id].append(
                CodexEvent(
                    event_type=CodexEventType.ERROR,
                    payload={'stderr': completed.stderr.strip(), 'returncode': completed.returncode},
                    session_id=session_id,
                )
            )
