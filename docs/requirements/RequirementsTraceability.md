...
The corresponding next-MVP test plan is: valid verify and fake implement; wrong or disabled target; unsupported version; malformed input; unauthorized caller; unsupported task type; invalid concurrency group; duplicate or conflicting delivery; matching/ambiguous/racing managed draft; fake Codex, validation, test, or publication failure; canonical result; receiver fail-closed response; identical/conflicting result redelivery; and proof of no Codex network call, real branch, or real PR. `TC-MVP-CI-001` supplies authoritative scenario names/coverage but not complete executable inputs/outputs, so this is planned coverage rather than a full shared-fixture conformance claim.
