...
class CanvaClient(ICanvaService):
    ...
    def list_designs(self) -> list[CanvaDesign]:
        response = requests.get(self._url('/designs'), headers=self._headers(), timeout=30)
        response.raise_for_status()
        items = response.json().get('items', [])
        return [self._parse_design(item) for item in items]

    def get_design(self, design_id: str) -> CanvaDesign:
        response = requests.get(self._url(f'/designs/{design_id}'), headers=self._headers(), timeout=30)
        response.raise_for_status()
        return self._parse_design(response.json().get('design', response.json()))
    ...
    def export_design(self, design_id: str, export_format: CanvaExportFormat) -> CanvaExportJob:
        ...
        response.raise_for_status()
        data = response.json().get('job', response.json())
    ...
    def list_folders(self) -> list[CanvaFolder]:
        response = requests.get(self._url('/folders'), headers=self._headers(), timeout=30)
        response.raise_for_status()
        items = response.json().get('items', [])
        return [
            CanvaFolder(
    ...
    def list_brand_templates(self) -> list[CanvaBrandTemplate]:
        response = requests.get(self._url('/brand-templates'), headers=self._headers(), timeout=30)
        response.raise_for_status()
        items = response.json().get('items', [])
        return [
            CanvaBrandTemplate(
