...
class ConfigLoader:
    ...
    def _apply_environment(self, config: dict[str, Any]) -> dict[str, Any]:
        config["environment"] = os.getenv(
            "SLUGGER_ENV", config.get("environment", "development")
        )
        workflow = dict(config.get("workflow", {}))
        workflow["recipe_directory"] = os.getenv(
            "SLUGGER_WORKFLOW_DIR", workflow.get("recipe_directory", "workflow/recipes")
        )
        ...
        env_canva_token = os.getenv(
            canva.get("access_token_env", "CANVA_API_TOKEN"), ""
        )
        if env_canva_token:
        ...
        env_openai_key = os.getenv(
            codex.get("fallback_api_key_env", "OPENAI_API_KEY"), ""
        )
        if env_codex_key:
    ...
    def _to_settings(self, data: dict[str, Any]) -> Settings:
        ...
        providers = ProviderSettings(
            default_provider=data.get("providers", {}).get("default_provider", "mock"),
            configs=provider_configs or ProviderSettings().configs,
        )
