...
For a manual GitHub Actions demo that follows the MVP certification flow but lets the operator supply the project idea, run **Actions → User Idea Codex Slugger MVP Demo**. Provide `idea` as the generated CLI concept and `project_name` as a lowercase kebab-case package name; the workflow renders a safe Codex prompt, generates the project in `generated-demo/`, validates it through Slugger, and uploads a reviewable artifact.
